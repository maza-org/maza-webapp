        ...
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: bottomSafeSpace(insets.bottom, 32) }}>
          ...
          {formOpen ? (
            <View style={[styles.formCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              {type === 'SELF_EMPLOYMENT' ? <View style={styles.chips}>{['INFORMAL', 'FORMAL'].map((value) => <TouchableOpacity key={value} style={[styles.chip, { borderColor: businessForm === value ? colors.primary : colors.border }]} onPress={() => setBusinessForm(value)}><Text style={{ color: businessForm === value ? colors.primary : colors.text }}>{value === 'FORMAL' ? 'Formal' : 'Informal'}</Text></TouchableOpacity>)}</View> : null}
